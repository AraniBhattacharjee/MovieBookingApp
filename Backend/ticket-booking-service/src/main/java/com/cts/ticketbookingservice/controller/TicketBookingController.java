...
public class TicketBookingController {
	...
	@Operation(summary = "View ticket", description = "View tickets")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Ticket found", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "400", description = "Bad request", content = @Content(schema = @Schema(implementation = Response.class))),
			@ApiResponse(responseCode = "404", description = "Tickets not found", content = @Content(schema = @Schema(implementation = Response.class))),
	...
	public ResponseEntity<TicketsDTO> viewTicket(
			@Parameter(description = "JWT auth token of the application admin", required = true) @RequestHeader(name = "Authorization") String token) {
		
		return ResponseEntity.ok(bookingService.viewTickets(token));
	}
